...
export default function PriceComparator({ show, onHide, cardId, cardName, cardImage }) {
  ...
  const fetchAllSellers = async () => {
    setLoading(true);
    try {
      // Buscar todos los listings de esta carta específica
      ...
      const listings = listingsSnapshot.docs.map(d => ({ id: d.id, ...d.data() }));

      // Obtener información de rating de cada vendedor
        ...
        listings.map(async (listing) => {
          try {
            ...
            return {
              ...
              sellerReviews: userData.reviews || 0,
              sellerJoinDate: userData.createdAt || null
            };
          } catch (error) {
      ...
      // Ordenar por precio
      sellersWithRatings.sort((a, b) => a.price - b.price);
      setSellers(sellersWithRatings);

      // Calcular estadísticas
      if (sellersWithRatings.length > 0) {
        const prices = sellersWithRatings.map(s => s.price);
        const avgPrice = prices.reduce((sum, price) => sum + price, 0) / prices.length;
        const minPrice = Math.min(...prices);
        const maxPrice = Math.max(...prices);
        
        setStats({
          totalSellers: sellersWithRatings.length,
